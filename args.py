...
def build_arg_parser():
    ...
    p.add_argument('--ensemble-table', action='store_true', help='Flags program to write ensemble results to tsv files. Appends to a filename based on path to the audio and the path to the model. You may need to delete rows from previous runs if they are not needed.')
    p.add_argument('--ensemble-json', action='store_true', help='Flags program to write ensemble results to json files. Appends to filename based on the path of the audio and the path to the model. You may need to delete the entries from previous runs if they are not needed.')
